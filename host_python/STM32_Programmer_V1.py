...
def decode_menu_command_code(command):
	...
	if(command	== 0 ):
		...
		sector_num = input("\n	 Enter page number(0-255 or 0x00 - 0xFF(ensure to use 0x for hex) or 0xFF for mass erase) here :")
		if("0x" in sector_num):
			sector_num = int(sector_num, 16)
		else:
			sector_num = int(sector_num)
		if(sector_num != 0xff):
			nsec=int(input("\n	 Enter number of pages to erase(max 255) here :"))
		else:
			nsec = int(255)
		...
		print("\n	Mode:Flash page Write Protection: 1")
		print("\n	Mode:Flash page Read/Write Protection: 2")
...
def read_bootloader_reply(command_code):
	...
	print()
	#ack1 = read_serial_port(len(ack1))
	#print(ack1)
	ack=read_serial_port(2)
	#print(ack)
	#print("The len of ack is ",len(ack))
	if(len(ack) ):
		a_array=bytearray(ack)
		#print("a_arrat is: ",a_array)
		#print("read uart:",ack) 
		if (a_array[0]== 0xA5):
...
while True:
	print("\n +==========================================+")
	print(" |				Menu					   |")
	print(" |	STM32F303RE BootLoader v1			   |")
	print(" +==========================================+")
